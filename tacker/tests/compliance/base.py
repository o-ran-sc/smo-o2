...
from tacker.tests.functional.sol_kubernetes.vnflcm.base import BaseVnfLcmKubernetesTest as cnflcmtest
            ...
            "extCps": [
                {
                    "cpdId": "CP1",
                    "cpConfig": [
                        {"cpProtocolData": [{"layerProtocol": "IP_OVER_ETHERNET"}]}
                    ],
                }
            ],
        }
...
class BaseComplTest(base.BaseTackerTest):
    ...
    @classmethod
    def _create_and_upload_kubernetes_vnf_packages(cls, pkgnames):
        vnfpkginfos = []
        for pkgname in pkgnames:
            vnfpkgid, vnfdid = cnflcmtest._create_and_upload_cnf_package(
                cls, cls.http_client, pkgname, {"key": "sample_1_functional"})
            vnfpkginfos.append(VnfPkgInfo(vnfpkgid, vnfdid))
    ...
    @classmethod
    def _instantiate_cnf_instance(cls, vnfid):
