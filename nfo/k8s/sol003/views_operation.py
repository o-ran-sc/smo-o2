...
class VnfLcmOperationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = VnfLcmOperation.objects.select_related('vnf_instance').order_by('-start_time')
    ...
    @action(detail=False, methods=['post'], url_path='operations/(?P<operation_id>[^/.]+)/retry')
    ...
    def _retry_instantiate_operation(self, instance, operation):
        ...
        result = self.deploy_kubernetes(instance, operation.operation_params, final_values)
        ...
        if result['success']:
            ...
            if self._is_retryable_error(error_type) and operation.retry_count < operation.max_retries:
    ...
    def _retry_update_operation(self, instance, operation):
        ...
        if result['success']:
            ...
            return {'success': False, 'message': f'Update retry failed: {result.get("error")}'}

    @action(detail=False, methods=['get'], url_path='operations/(?P<operation_id>[^/.]+)')
    def get_operation(self, request, operation_id=None):
        ...
        try:
            ...
            if operation.retry_count > 0:
                ...
                response_data['executionAttempts'] = getattr(operation, 'execution_attempts', [])
    ...
    def _retry_terminate_operation(self, instance, operation):
        ...
        try:
            ...
            if result['success'] or force:
                ...
                if self._is_retryable_error(error_type) and operation.retry_count < operation.max_retries:
                    ...
                    if error_type == 'not_found':
                        ...
                        logger.warning("Resources not found during termination retry - considering as success")
                        instance.instantiation_state = 'NOT_INSTANTIATED'
                        instance.deployment_namespace = None
                        instance.helm_release_name = None
                        instance.allocated_machines.clear()
            ...
            if force:
                logger.warning(f"Force termination: cleaning up despite error: {str(e)}")
                instance.instantiation_state = 'NOT_INSTANTIATED'
                instance.deployment_namespace = None
                instance.helm_release_name = None
                instance.allocated_machines.clear()
