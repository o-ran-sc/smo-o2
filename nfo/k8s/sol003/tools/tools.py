        ...
        for key, value in data.items():
            ...
            for i, k in enumerate(keys):
                ...
                if '[' in k and ']' in k:
                    ...
                    if base_key not in current: current[base_key] = []
...
class KubernetesResourceMetrics:
        ...
        try:
            ...
            for item in result.get('data', {}).get('result', []):
                parsed.append({
                    ...
                    'timestamp': datetime.fromtimestamp(item.get('value', [0, 0])[0])
                })
            return parsed
    ...
    def _calculate_available(self, results: Dict) -> Dict:
        ...
        for metric_name, data_list in results.items():
            for data in data_list:
                # Get node identifier
                node = data['labels'].get('node') or data['labels'].get('instance', 'unknown')
        ...
        for node, metrics in nodes.items():
            ...
            if 'cpu_allocatable_millicores' in metrics and 'cpu_requested_millicores' in metrics:
                metrics['cpu_available_millicores'] = metrics['cpu_allocatable_millicores'] - metrics.get('cpu_requested_millicores', 0)
                metrics['cpu_available_cores'] = metrics['cpu_available_millicores'] / 1000
            
            # Memory Available (already calculated by node exporter)
            if 'memory_allocatable' in metrics and 'memory_requested' in metrics:
                metrics['memory_unrequested_bytes'] = metrics['memory_allocatable'] - metrics.get('memory_requested', 0)
                metrics['memory_unrequested_gi'] = metrics['memory_unrequested_bytes'] / (1024**3)
            ...
            if 'pods_allocatable' in metrics and 'pods_running' in metrics:
                metrics['pods_available'] = metrics['pods_allocatable'] - metrics.get('pods_running', 0)
    ...
    def get_network_details(self) -> List[Dict]:
        ...
        virtual_patterns = [
            'lo', 'docker', 'veth', 'virbr', 'vir', 'tun', 'tap',
            'cali', 'flannel', 'cni', 'kube', 'dummy', 'bond',
            'br-', 'vxlan', 'weave', 'datapath'
        ]
        ...
        for instance, devices in network_data.items():
            for device, metrics in devices.items():
                ...
                if speed >= 125000000 or (mac and mac != '00:00:00:00:00:00'):  # 125MB/s = 1Gbps
                    flattened.append({
                        'node': instance,
                        'interface': device,
                        'up': metrics.get('interface_up', 0) == 1,
                        'speed_gbps': speed * 8 / (1000**3) if speed else 0,
                        'rx_mbps': metrics.get('receive_bytes_rate', 0) * 8 / (1000**2),
                        'tx_mbps': metrics.get('transmit_bytes_rate', 0) * 8 / (1000**2),
                        'mac': mac,
    ...
    def get_cluster_summary(self) -> Dict:
        ...
        totals = {
            'cpu': {
                'capacity_cores': sum(n.get('cpu_capacity', 0) for n in node_resources.values()),
                'allocatable_cores': sum(n.get('cpu_allocatable', 0) for n in node_resources.values()),
                'available_millicores': sum(n.get('cpu_available_millicores', 0) for n in node_resources.values()),
                'requested_millicores': sum(n.get('cpu_requested_millicores', 0) for n in node_resources.values()),
                'cpu_threads_total': sum(n.get('cpu_threads', 0) for n in node_resources.values())
            },
            'memory': {
                'capacity_gi': sum(n.get('memory_capacity', 0) for n in node_resources.values()) / (1024**3),
                'allocatable_gi': sum(n.get('memory_allocatable', 0) for n in node_resources.values()) / (1024**3),
                'available_gi': sum(n.get('memory_available', 0) for n in node_resources.values()) / (1024**3),
                'unrequested_gi': sum(n.get('memory_unrequested_bytes', 0) for n in node_resources.values()) / (1024**3)
            },
            'pods': {
                'capacity': sum(n.get('pods_capacity', 0) for n in node_resources.values()),
                'running': sum(n.get('pods_running', 0) for n in node_resources.values()),
                'available': sum(n.get('pods_available', 0) for n in node_resources.values())
            },
            'network': {
                ...
                'dpdk_capable_nodes': nic_capabilities['cluster_capabilities']['dpdk_nodes'],
                'sriov_capable_nodes': nic_capabilities['cluster_capabilities']['sriov_nodes'],
                'total_dpdk_nics': nic_capabilities['cluster_capabilities']['total_dpdk_nics'],
                'total_sriov_nics': nic_capabilities['cluster_capabilities']['total_sriov_nics'],
        ...
        for node_name in node_resources.keys():
            if node_name in nic_capabilities['per_node']:
                node_resources[node_name]['nic_capabilities'] = {
                    'dpdk_supported': nic_capabilities['per_node'][node_name]['dpdk_supported'],
                    'sriov_supported': nic_capabilities['per_node'][node_name]['sriov_supported'],
                    'dpdk_nics': nic_capabilities['per_node'][node_name]['dpdk_capable'],
                    'sriov_nics': nic_capabilities['per_node'][node_name]['sriov_capable'],
                    'hugepages_configured': nic_capabilities['per_node'][node_name]['hugepages_configured'],
                    'iommu_enabled': nic_capabilities['per_node'][node_name]['iommu_enabled']
                }
    ...
    def _format_readable_summary(self, totals: Dict, nic_capabilities: Dict = None) -> Dict:
    ...
    def check_nic_capabilities(self) -> Dict:
        ...
        for item in results.get('pci_devices', []):
            ...
            has_speed = (node in physical_nics and device in physical_nics[node] and 
                        physical_nics[node][device] > 0)
            ...
            dpdk_drivers = [
                ...
                'ixgbe', 'i40e', 'ice', 'igb', 'e1000e', 'e1000',
                'mlx4_core', 'mlx5_core', 'mlx4_en', 'mlx5_en',
                'bnx2x', 'bnxt_en', 'qede', 'nfp'
            ]
        ...
        summary = {
            'cluster_capabilities': {
                'dpdk_nodes': sum(1 for n in node_capabilities.values() if n['dpdk_supported']),
                'sriov_nodes': sum(1 for n in node_capabilities.values() if n['sriov_supported']),
                'total_dpdk_nics': sum(len(n['dpdk_capable']) for n in node_capabilities.values()),
                'total_sriov_nics': sum(len(n['sriov_capable']) for n in node_capabilities.values()),
                'total_physical_nics': sum(len(n['interfaces']) for n in node_capabilities.values()),
                'hugepages_nodes': sum(1 for n in node_capabilities.values() if n['hugepages_configured']),
                'iommu_nodes': sum(1 for n in node_capabilities.values() if n['iommu_enabled'])
            },
