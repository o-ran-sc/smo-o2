...
class NfDeploymentInstanceViewSet(viewsets.ModelViewSet):
            ...
            return Response(
                {
                    ...
                    "rollbackExecuted": (
                        ...
                        if hasattr(operation, "rollback_executed")
                        else False
                    ),
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    def deploy_kubernetes(self, instance, params, values):
        ...
        try:
            ...
            logger.error("**********************************************************8")
            try:
                k8s_cluster = KubernetesCluster.objects.get(
                    cloud_id=instance.descriptor.target_cluster_id
                )
            except KubernetesCluster.DoesNotExist:
                return {
                    "success": False,
                    "error": "No Kubernetes configuration found for cluster",
                }
            ...
            if descriptor.artifact_repo_url and descriptor.artifact_name:
                ...
                if result["success"]:
                    ...
                    k8s_deployment.error_message = result.get(
                    ...
                    if (
                        "port" in str(result.get("error", "")).lower()
                        and "allocated" in str(result.get("error", "")).lower()
                    ):
                ...
                if manifests:
                    deployed_manifests = k8s_service.deploy_manifests(
                        namespace, manifests
                    )
    ...
    def terminate_kubernetes(
        self, instance, graceful=True, force=False, cleanup_namespace=True
    ):
        ...
        try:
            ...
            if cleanup_namespace and instance.deployment_namespace:
                ns_result = k8s_service.delete_namespace(
                    namespace=instance.deployment_namespace, force=force
                )
                termination_steps.append(
                    {
            ...
            return {
                ...
                "message": (
                    ...
                    if all_success
                    else "Termination completed with errors"
                ),
            }
            ...
            if force:
                return {
                    "success": True,
                    "message": f"Force terminated despite error: {str(e)}",
                }
            return {"success": False, "error": str(e)}
    ...
    def upgrade(self, request, instance_id=None):
        ...
        operation = VnfLcmOperation.objects.create(
            ...
            operation_params={"values": new_values, "previous_values": stored_values},
        )
        ...
        for instance_id in instance_ids:
            try:
                ...
                results["failed"].append(
                    {"instance_id": str(instance_id), "error": "Instance not found"}
                )
            except Exception as e:
                results["failed"].append(
    ...
    def _execute_with_retry(self, operation, deployment_func, *args, **kwargs):
        ...
        while retry_count <= max_retries:
            ...
            try:
                ...
                error_type = result.get("error_type", "unknown")
                if (
                    not self._is_retryable_error(error_type)
    ...
    def _execute_rollback(self, instance, operation):
        ...
        logger.error(
            f"Executing automatic rollback for operation {operation.operation_id}"
        )
        ...
        try:
            if operation.operation_type == "INSTANTIATE":
                ...
                if operation.state_snapshot:
                    previous_values = operation.state_snapshot.get(
                        "k8s_deployment", {}
                    ).get("helm_values", {})
...
class KubernetesClusterViewSet(viewsets.ModelViewSet):
    ...
    def set_credentials(self, request, pk=None):
        ...
        serializer = KubernetesClusterCredentialsSerializer(
            k8s_cluster, data=request.data
        )
...
class DeploymentDescriptorListViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        by_profile = self.queryset.values("profile_type").annotate(
            count=Count("profile_type")
        )
...
class NfDeploymentInstanceListViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request):
        ...
        total = self.queryset.count()
        by_state = self.queryset.values("instantiation_state").annotate(
            count=Count("instantiation_state")
        )
        by_profile = self.queryset.values("descriptor__profile_type").annotate(
            count=Count("descriptor__profile_type")
        )
        ...
        ).aggregate(avg_time=Avg("end_time") - Avg("start_time"))
    ...
    def failed(self, request):
        ...
        for instance in failed_instances:
            last_operation = (
                instance.operations.filter(operation_state="FAILED")
                .order_by("-start_time")
                .first()
            )
            ...
            failed_data.append(
                {
                    ...
                    "error_details": (
                        last_operation.error_details if last_operation else {}
                    ),
                    "last_operation": (
                        {
        ...
        return Response(
            {"failed_count": len(failed_data), "failed_instances": failed_data}
        )
...
class KubernetesDeploymentListViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request):
        ...
        total = self.queryset.count()
        by_status = self.queryset.values("kubernetes_status").annotate(
            count=Count("kubernetes_status")
        )

        # Health status
        healthy = self.queryset.filter(
            kubernetes_status="RUNNING", pods_ready__gte=models.F("pods_desired")
        ).count()
    ...
    def homing(self, request):
        ...
        for cluster in self.get_queryset():
            try:
                ...
                cpu_available = (
                    summary["cluster_totals"]["cpu"]["available_millicores"] / 1000
                )
                ...
                score = (cpu_available - cpu_needed) + (
        ...
        return Response(
            {
                ...
                "requirements": {"cpu_cores": cpu_needed, "memory_gi": memory_needed},
